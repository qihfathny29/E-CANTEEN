...
@section('page-styles')
        ...
        .saldo-row{display:flex;gap:.6rem;align-items:stretch;flex-wrap:wrap;}
        .saldo-row .form-control{flex:1;min-width:150px;}
    ...
    <div class="form-card">
        <form method="POST" action="{{ route('admin.users.update', $user) }}">
            ...
            <div class="form-group">
                ...
                <div class="radio-group">
                    <div class="radio-card">
                        <input type="radio" id="type_siswa" name="user_type" value="siswa"
                            {{ old('user_type', $user->user_type) === 'siswa' ? 'checked' : '' }}>
                        <label for="type_siswa"><i class="fas fa-user-graduate" style="font-size:.75rem;"></i> Siswa</label>
                    </div>
                    <div class="radio-card">
                        <input type="radio" id="type_guru" name="user_type" value="guru"
                            {{ old('user_type', $user->user_type) === 'guru' ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <div class="form-hint">Minimal 8 karakter. Biarkan kosong jika tidak ingin mengganti password.</div>
            ...
            <div class="form-group">
                ...
                <div class="saldo-row">
                    <input id="saldo" type="number" name="saldo" class="form-control" value="{{ old('saldo', $user->saldo) }}" min="0" step="1000">
                </div>
                <div class="form-hint">Untuk top-up saldo, gunakan halaman <a href="{{ route('admin.saldo.index') }}" style="color:var(--red);">Kelola Saldo</a>. Edit langsung di sini hanya untuk koreksi manual.</div>
                @error('saldo')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="btn-row">
                <button type="submit" class="btn-submit"><i class="fas fa-save" style="font-size:.75rem;"></i> Simpan Perubahan</button>
